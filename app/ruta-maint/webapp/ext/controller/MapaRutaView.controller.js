sap.ui.define([
  ...
  return Controller.extend("rutamaint.ext.controller.MapaRutaView", {
    ...
    _getMapContainer: function () {
      ...
      var oMapDiv = document.getElementById("mapa-ruta-container");
      if (oMapDiv) {
      ...
      if (!oMapDiv) {
        ...
        oMapDiv.id = "mapa-ruta-container";
        oMapDiv.style.width = "100%";
        oMapDiv.style.height = "480px";
        oMapDiv.style.minHeight = "480px";
        oMapDiv.style.position = "relative";
    ...
    _initMap: function (oContainer) {
      ...
      this._getRouteContext().then(function (oRoute) {
        _log("Contexto de ruta obtenido", oRoute);

        return that._extractPoints(oRoute);
      }).then(function (aPoints) {
        _log("Puntos extraídos", aPoints);

        if (!aPoints || aPoints.length === 0) {
          that._showMessage(oContainer, "No se encontraron puntos para esta ruta.");
          return;
        }
        ...
        _error("Error leyendo contexto de ruta", oErr);
        that._bInitializing = false;
        that._showMessage(oContainer, "Error al leer los datos de la ruta.");
      });
    },
    ...
    _getRouteContext: function () {
      ...
      return new Promise(function (resolve) {
        ...
        if (oContext) {
          requestObject(oContext).then(function (oRoute) {
            _log("requestObject resultado", oRoute);
            resolve(oRoute);
          });
          return;
        }
        ...
        var iInterval = setInterval(function () {
          ...
          if (oContext) {
            ...
            requestObject(oContext).then(resolve);
    ...
    _extractPoints: function (oRoute) {
      function isMaracaibo(oP) {
        if (!oP || !oP.descripcion) {
          return false;
        }
        return String(oP.descripcion).toLowerCase().indexOf("maracaibo") !== -1;
      }

      function normalizePoints(aPuntos) {
        if (!Array.isArray(aPuntos) || aPuntos.length === 0) {
          return [];
        }

        var aValid = aPuntos.map(function (oP) {
          return {
            descripcion: oP.descripcion || "Punto",
            lat: parseFloat(oP.latitud),
            lng: parseFloat(oP.longitud),
            orden: typeof oP.orden === "number" ? oP.orden : parseInt(oP.orden, 10),
            createdAt: oP.createdAt || ""
          };
        }).filter(function (oP) {
          return !isNaN(oP.lat) && !isNaN(oP.lng);
        });

        // Ordenar primero por el campo orden si está disponible;
        // de lo contrario, dejar el orden original del backend.
        var bHasOrden = aValid.some(function (oP) { return !isNaN(oP.orden); });
        if (bHasOrden) {
          aValid.sort(function (a, b) {
            var iA = isNaN(a.orden) ? Number.MAX_SAFE_INTEGER : a.orden;
            var iB = isNaN(b.orden) ? Number.MAX_SAFE_INTEGER : b.orden;
            return iA - iB;
          });
        }

        // Asegurar que Maracaibo sea siempre el origen (primer marcador).
        var iMaracaibo = aValid.findIndex(isMaracaibo);
        if (iMaracaibo > 0) {
          var oMaracaibo = aValid.splice(iMaracaibo, 1)[0];
          aValid.unshift(oMaracaibo);
        }
      ...
      if (Array.isArray(oRoute.puntos)) {
        return Promise.resolve(normalizePoints(oRoute.puntos));
      }

      // Si la asociación no vino expandida, intentar leerla del modelo
      var vPuntos = this._readPointsFromContext();
      if (vPuntos && typeof vPuntos.then === "function") {
        return vPuntos.then(normalizePoints);
      }

      return Promise.resolve(normalizePoints(vPuntos));
    },

    _readPointsFromContext: function () {
      var oContext = this._findContext();
      if (!oContext) {
        return null;
      }
      ...
      function requestWithSelect(sSelect) {
        var oListBinding = oModel.bindList(sPath + "/puntos", oContext, [], [], {
          $$operationMode: "Server",
          $select: sSelect
        });
        if (oListBinding && typeof oListBinding.requestContexts === "function") {
      ...
      try {
        return requestWithSelect("orden,latitud,longitud,descripcion").catch(function (oErr) {
          _warn("No se pudieron leer puntos con 'orden'; reintentando sin ese campo", oErr);
          return requestWithSelect("latitud,longitud,descripcion");
        });
      } catch (oErr) {
        _warn("No se pudo leer puntos desde el modelo", oErr);
      }
    ...
    _renderMap: function (google, oContainer, aPoints, sMapId) {
      ...
      // Dibujar marcadores para todos los puntos
      aPoints.forEach(function (oPoint, iIndex) {
        var sLabel = String(iIndex + 1);
        var sColor = iIndex === 0 ? "#0070b1" : (iIndex === aPoints.length - 1 ? "#107e3e" : "#e9730c");
